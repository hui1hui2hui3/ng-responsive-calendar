        ...
        return {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                ctrl._refreshView = function () {
                    ...
                    /*修改原因：修改月视图为中文，修改人：yd*/
                    scope.labels   = ['日','一','二','三','四','五','六'];
            ...
            link: function (scope, element, attrs, ctrl) {
                    ...
                    for (var i = 0; i < len; i += 1) {
                        ...
                        if (event.allDay) {
                            ...
                            if (eventEndTime <= startTime || eventStartTime >= endTime) {
                                ...
                                var startTop ;
                                var subHeight;
                                if(eventStartTime.getMinutes()==0){
                                    startTop = 0;
                                }else if(eventEndTime.getMinutes()==0){
                                    subHeight = 0;
                                }else{
                                    startTop = eventStartTime.getMinutes()/60;
                                    subHeight = (60-eventEndTime.getMinutes())/60;
                                }
                                ...
                                do {
                                    ...
                                    if((event.startTime.getFullYear()===event.endTime.getFullYear())&&
                ...
                ctrl._refreshView = function () {
                    var firstDayOfWeek = ctrl.range.startTime,
                        //modify yd
                        endDayOfWeek = ctrl.range.endTime,
					...
					if(dateFilter(firstDayOfWeek, ctrl.formatWeekTitle)===dateFilter(endDayOfWeek, ctrl.formatWeekTitle)){
					title = dateFilter(firstDayOfWeek, ctrl.formatWeekTitle);
					}else{
                    title = dateFilter(firstDayOfWeek, ctrl.formatWeekTitle)+"-"+dateFilter(endDayOfWeek, ctrl.formatWeekTitle);
					}
...
angular.module("template/rcalendar/calendar.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("template/rcalendar/calendar.html",
            "<div ng-switch=\"calendarMode\" class=\"kyeeCalendarCls\">\n" +
            /*yd modify */
            //"    <div class=\"row calendar-navbar\">\n" +
            "    <div class=\"rowCls calendar-navbar\">\n" +
            "        <div class=\"nav-left col-xs-2\">\n" +
			/*modify yd*/
			"            <button type=\"button\" class=\"button icon-left ion-chevron-left\" ng-click=\"move(-1)\">\n" +
			"            </button>\n"+
			...
			/*modify yd*/
			"            <button type=\"button\" class=\"button icon-right ion-chevron-right\" ng-click=\"move(1)\">\n" +
			"            </button>\n"+
...
angular.module("template/rcalendar/day.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("template/rcalendar/day.html",
            ...
            "                        </div>\n" +
            "                    </td>\n" +
            /*yd modify allDay table right div width style*/
//            "                    <td ng-if=\"allDayEventGutterWidth>0\" ng-style=\"{width:allDayEventGutterWidth+'px'}\"></td>\n" +
            "                    <td ng-if=\"allDayEventGutterWidth>0\" ng-style=\"width:'0px'}\"></td>\n" +
            "                </tr>\n" +
            "                </tbody>\n" +
            "            </table>\n" +
            "        </div>\n" +
            "    </div>\n" +
            /*yd month view modify*/
//    "    <div class=\"scrollable\" style=\"height: 400px\">\n" +
            "<ion-content style=\"top: 93px;bottom: 50px;width:100%;height:390px;\"  class=\"kyeeDayViewCls\">\n" +
            ...
            "                             ng-click=\"eventSelected({event:displayEvent.event})\"\n" +
            "                             ng-style=\"{left: 100/displayEvent.overlapNumber*displayEvent.position+'%', width: 100/displayEvent.overlapNumber+'%', height: 38*(displayEvent.endIndex-displayEvent.startIndex-displayEvent.startTop-displayEvent.subHeight)+'px',top:38*displayEvent.startTop+'px'}\">\n" +
            "                            <div class=\"calendar-event-inner\">{{displayEvent.event.title}}</div>\n" +
            "                        </div>\n" +
            "                    </div>\n" +
            "                </td>\n" +
...
angular.module("template/rcalendar/month.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("template/rcalendar/month.html",
            ...
            "                    </td>\n" +
                /*modify yd*/
            "                    <td ng-if=\"event.allDay\" class=\"monthview-eventdetail-timecolumn\">全天</td>\n" +
            "                    <td class=\"event-detail\" ng-click=\"eventSelected({event:event})\">{{event.title}}</td>\n" +
            "                </tr>\n" +
                /*modify yd*/
            "                <tr ng-if=\"!selectedDate.events\"><td class=\"no-event-label\">无事件</td></tr>\n" +
            "            </table>\n" +
            "        </div>\n" +
            "    </div>\n" +
            "</div>");
...
angular.module("template/rcalendar/week.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("template/rcalendar/week.html",
            ...
            "            <th class=\"calendar-hour-column\"></th>\n" +
                /*修改原因：周视图表头样式，修改人：yd*/
                "            <th ng-repeat=\"dt in dates\" class=\"text-center weekview-header-label\" style='padding:0;'><div>{{['日','一','二','三','四','五','六'][$index]}}</div><div style='font-size:14px'>{{dt.date| date: 'd'}}</div></span></th>\n" +
...
//            "                                 ng-style=\"{top: 25*displayEvent.position+'px')+'%',width: '100%', height: '25px'}\">\n" +
            ...
            /*yd dayView modify*/
//            "    <div class=\"scrollable\" style=\"height: 400px\">\n" +
            "<ion-content style=\"top: 132px;bottom: 50px;width:100%;height:345px;\"  class=\"kyeeWeekViewCls\">\n" +
            "        <table class=\"table table-bordered table-fixed\">\n" +
            "            <tbody>\n" +
            "            <tr ng-repeat=\"row in rows track by $index\">\n" +
            "                <td class=\"calendar-hour-column text-center\">\n" +
